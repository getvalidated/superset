 ...
 *
 * tRPC HTTP queries are GET requests with the SuperJSON-encoded input in
 * the `input` query param; the response envelope matches `relayMutation`.
 */
export async function relayQuery<TInput, TOutput>(
	...
	const encoded = SuperJSON.serialize(input);
	const url = `${options.relayUrl}/hosts/${options.hostId}/trpc/${procedure}?input=${encodeURIComponent(JSON.stringify(encoded))}`;
