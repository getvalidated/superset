...
export const Route = createFileRoute("/companies/$domain/")({
	// /companies/$orgId was the accounts detail URL before the rename — old
	// bookmarks land here with a UUID where a domain belongs.
	beforeLoad: ({ params }) => {
		if (UUID_PATTERN.test(params.domain)) {
			throw redirect({
				to: "/accounts/$orgId",
				params: { orgId: params.domain },
			});
		}
	},
